...
class RealESRGANDataset(data.Dataset):
    ...
    def __init__(self, conf, mode='training'):
        ...
        self.paths = []
        if 'dir_paths' in conf:
            for current_dir in conf['dir_paths']:
                for current_ext in conf['im_exts']:
                    self.paths.extend(sorted([str(x) for x in Path(current_dir).glob(f'**/*.{current_ext}')]))
        if 'txt_file_path' in conf:
            for current_txt in conf['txt_file_path']:
                self.paths.extend(readline_txt(current_txt))
        if 'length' in conf:
            self.paths = random.sample(self.paths, conf['length'])

        # blur settings for the first degradation
        self.blur_kernel_size = conf['blur_kernel_size']
        self.kernel_list = conf['kernel_list']
        self.kernel_prob = conf['kernel_prob']  # a list for each kernel probability
        self.blur_sigma = conf['blur_sigma']
        self.betag_range = conf['betag_range']  # betag used in generalized Gaussian blur kernels
        self.betap_range = conf['betap_range']  # betap used in plateau blur kernels
        self.sinc_prob = conf['sinc_prob']  # the probability for sinc filters

        # blur settings for the second degradation
        self.blur_kernel_size2 = conf['blur_kernel_size2']
        self.kernel_list2 = conf['kernel_list2']
        self.kernel_prob2 = conf['kernel_prob2']
        self.blur_sigma2 = conf['blur_sigma2']
        self.betag_range2 = conf['betag_range2']
        self.betap_range2 = conf['betap_range2']
        self.sinc_prob2 = conf['sinc_prob2']

        # a final sinc filter
        self.final_sinc_prob = conf['final_sinc_prob']

        self.kernel_range1 = [x for x in range(3, conf['blur_kernel_size'], 2)]  # kernel size ranges from 7 to 21
        self.kernel_range2 = [x for x in range(3, conf['blur_kernel_size2'], 2)]  # kernel size ranges from 7 to 21
        # TODO: kernel range is now hard-coded, should be in the configure file
        # convolving with pulse tensor brings no blurry effect
        self.pulse_tensor = torch.zeros(conf['blur_kernel_size2'], conf['blur_kernel_size2']).float()
        self.pulse_tensor[conf['blur_kernel_size2']//2, conf['blur_kernel_size2']//2] = 1
    ...
    def degrade_fun(self,  im_gt, kernel1, kernel2, sinc_kernel, conf_degradation=None,):
        ...
        if len(ori_shape) == 3:
            im_lq = im_lq.squeeze(0)
            im_gt = im_gt.squeeze(0)
