...
def main(args):
    ...
    metric_psnr = torch.nn.MSELoss()
    metric_ssim = pyiqa.create_metric("ssim", as_loss=True)
    metric_lpips = pyiqa.create_metric("lpips", as_loss=True)

    def get_pixel_loss(input, target, psnr_fact=0.1, ssim_fact=0.1, lpips_fact=0.1):
        psnr_loss = metric_psnr(input, target)
        ssim_loss = 1 - metric_ssim(input, target)
        lpips_loss = metric_lpips(input, target)

        return psnr_fact * psnr_loss + ssim_fact * ssim_loss + lpips_fact * lpips_loss
    ...
    model = DiT_models[args.model](
        ...
        grad_checkpoint = args.grad_ckpt
    )
    ...
    if args.compile:
        ...
        vae.decode = torch.compile(vae.decode, fullgraph=True, mode="max-autotune")
    ...
    for epoch in range(100000000000):
        ...
        for batch in loader:
            ...
            with torch.autocast("cuda", enabled=True):
                ...
                if args.pixel_loss > 0 and train_steps > args.pixel_loss:

                    pred = loss_dict["model_outputs"]
                    pred = pout(pred)
                    
                    with torch.no_grad(): pred = vae.decode(pred).sample
                    
                    pred = pred / 2 + 0.5
                    targ = x_ / 2 + 0.5


                    pix_loss = get_pixel_loss(pred, targ, 0.1, 0.1, 0.01)
                    loss += pix_loss

                    if random.random() < 0.01:
                        print(f"pix_loss: {pix_loss.item()}")

                        # save images
                        save_pred = pred[0].clamp(0, 1) * 255
                        save_targ = targ[0].clamp(0, 1) * 255
                        save_pred = save_pred.to(torch.uint8).cpu().numpy().transpose(1, 2, 0)
                        save_targ = save_targ.to(torch.uint8).cpu().numpy().transpose(1, 2, 0)
                        save_pred = Image.fromarray(save_pred)
                        save_targ = Image.fromarray(save_targ)
                        save_pred.save(f"pred.png")
                        save_targ.save(f"targ.png")                    
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pixel-loss", type=int, default=-1)
