...
def main(args):
    ...
    if args.checkpoint is not None:
        ...
        loaded_ema = model.load_state_dict(checkpoint["model"], strict=False)
        ema = deepcopy(model).to(device)
        ema.load_state_dict(checkpoint["ema"])
        loaded_params = model.load_state_dict(checkpoint["model"], strict=False)
        logger.info(f"Loaded model from {args.checkpoint}")
        logger.info(f"history: {str(loaded_params)}")
    ...
    warmup_scheduler = LinearLR(opt, start_factor=0.01, end_factor=1.0, total_iters=20000)
