# Copyright 2024 Huage001/LinFusion
#
    ...
    def forward(
        ...
        **kwargs,
        ...
        if random.random() < 0.0001:
            print(f"hidden_states: max{torch.max(hidden_states)}, min{torch.min(hidden_states)}, avg{torch.mean(hidden_states)}")
            print(f"query: max{torch.max(query)}, min{torch.min(query)}, avg{torch.mean(query)}")
            print(f"key: max{torch.max(key)}, min{torch.min(key)}, avg{torch.mean(key)}")
