...
def main(args):
    ...
    constant = ConstantLR(opt, 1e-4)
    warmup_scheduler = LinearLR(opt, start_factor=.1, end_factor=1, total_iters=10000) 
    annealing_scheduler = CosineAnnealingLR(opt, T_max=120_000, eta_min=1e-5) # 80000 iters 从 1e-4 -> 1e-5
    sch = SequentialLR(opt, schedulers=[constant, warmup_scheduler, annealing_scheduler], milestones=[10000, 20000]) # 前10000 iters 恒定为 1e-5 (乘以1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--iters", type=int, default=120_100)
